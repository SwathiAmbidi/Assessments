...
public class CustomProductRepositoryImpl implements CustomProductRepository {
	...
	public List<Product> getProductByCategoryAndPrice(String proudctCategory, int range1, int range2) {
		...
		q.setParameter("productCategory", proudctCategory.equalsIgnoreCase(proudctCategory));
	...
	public List<Product> productsById() {
		//String qq = "Select b from Product as b order by b.id";
		Query q = entityManager.createQuery("from Product order by Product.Id");
		//q.setParameter("productCategory", proudctCategory);
		return q.getResultList();
	}
